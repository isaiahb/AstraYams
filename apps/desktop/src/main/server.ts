...
export async function startServer(options:{root?:string,dist?:string,port?:number}={}) {
 ...
 async function mcp(request:any,projectId?:string){const project=projectFor(state,projectId);
 ...
 const server=Bun.serve({hostname:'127.0.0.1',port:options.port||0,idleTimeout:60,async fetch(req){
  ...
  try{
   ...
   if(path==='/api/agents'&&req.method==='POST'){
    ...
    const result=await codex.request('thread/start',{cwd:project.directory,model:body.model||undefined,approvalPolicy:'on-request',sandbox:'workspace-write',developerInstructions:`You are the ${body.role} for AstraFactory. Work only on the assigned task. Respect other engineers' files. Publish inspectable artifacts and distinguish proposed, tested and completed work. Do not contact people or purchase resources. Project: ${project.title}. Project brief: ${project.brief}. Write new work inside ${project.directory}. MCP paths are relative to the repository root (${root}); use list_artifacts to discover them. Use AstraFactory MCP publish_artifact to register completed files with their workstream and stage. Research may be needed in any discipline. Simulations must state assumptions and feed proposed changes back to the responsible engineer. Do not invent experiment results.`,config:{'mcp_servers.astrafactory':{url:`http://127.0.0.1:${server.port}/mcp?projectId=${project.id}`,http_headers:{Authorization:`Bearer ${token}`}}}});
    const agent={id:result.thread.id,projectId:project.id,role:body.role,title:body.title||body.role,status:'starting',model:result.model,created:new Date().toISOString(),turnId:null};state.agents.push(agent);await save();
   ...
   if(path==='/mcp'){if(req.method!=='POST')return new Response(null,{status:405});return mcp(await req.json(),url.searchParams.get('projectId')||undefined);}
