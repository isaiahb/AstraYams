...
The keyed-insertion prepared prompt now continues into **fresh behavioral-cloning training and full-chain paired evaluation**. It starts from a newly initialized grasp model, uses prepared teacher data, and preserves downstream controllers. The UI clears inherited scores/replays before this run and only accepts a fresh `learning/demo.json` plus `report.accepted` result. Training/evaluation progress comes from the runner's JSONL events; the process is bounded to ten minutes. Machine and manual templates retain their prior setup-only behavior. Existing example cards retain their original evidence.
