...
def render(data, root, out):
    ...
    visual_hero = ''
    visual_candidates = [r for r in ordered if r['experiment_id'].startswith('visual-student-v') and r['experiment_id'].endswith('-retention-trained') and r.get('result')]
    visual = max(visual_candidates, key=lambda r: max(e['sequence'] for e in r['events'] if e['event_type']=='result')) if visual_candidates else None
    if visual:
        prefix = visual['experiment_id'].rsplit('-trained',1)[0]
        blank = by_id.get(prefix+'-blank-camera'); initial = by_id.get(prefix+'-untrained')
        result = visual['result']; videos = [a for a in evidence(visual) if a['path'].endswith('.mp4')]
        success_video = next((a for a in videos if a['path'].endswith('selected-success.mp4')), None)
        failure_video = next((a for a in videos if a['path'].endswith('sensor-video.mp4')), None)
        metrics = []
        ...
        count = len(visual['proposal']['data']['training_seeds'])
        visual_hero = '<section class="hero"><p class="eyebrow">LATEST VISUAL PICKUP EXPERIMENT · NOT PROMOTED</p><h1>Visual pickup remains unreliable.</h1><div class="metrics">'+''.join(metrics)+'</div><p>The '+f"{result['model']['parameter_count']:,}"+'-parameter student trained for '+f"{result['training']['wall_seconds']:.1f}"+' s on '+str(count)+' demonstrations. It receives two fixed RGB views plus joint and controller feedback. The trained candidate had '+str(result['force_aborts'])+' force-limit aborts in '+str(result['episodes'])+' trials.</p><p><strong>Software information-boundary audits passed; a visual benefit is not demonstrated.</strong> The blank-camera control matched or exceeded the trained RGB outcome on this small paired set. No full visual assembly or hardware transfer is established, and final seeds remain untouched.</p>'
        clips=[]
        if success_video:clips.append('<figure>'+player(success_video)+'<figcaption>Selected RGB success · '+str(result['successes'])+'/'+str(result['episodes'])+' total RGB trials succeeded</figcaption></figure>')
        if failure_video:clips.append('<figure>'+player(failure_video)+'<figcaption>Recorded failed RGB trial · '+str(result['episodes']-result['successes'])+'/'+str(result['episodes'])+' trials failed the pickup criterion</figcaption></figure>')
        if clips:visual_hero += '<div class="video-grid">'+''.join(clips)+'</div>'
        visual_hero += '<p><a href="#run-'+esc(visual['experiment_id'])+'">Inspect candidate, all outcomes and audits</a> · <a href="#run-'+esc(prefix+'-blank-camera')+'">Inspect blank-camera control</a> · <a href="#run-visual-student-v1-8episodes-retention-trained">Earlier v1: 0/5 trained pickup</a></p><details><summary>Student sensor contract and evidence</summary><p>RGB[2,160,160,3] + measured q7/qd7/commanded targets7/previous actions7. No object pose, contact-force or task-phase input; no runtime teacher or IK. Hardware calibration, timing and gripper mapping remain unvalidated.</p><pre>'+esc(json.dumps(visual,indent=2))+'</pre></details></section>'
        hero='<h2>Earlier privileged-state chain — separate scope</h2>'+hero.replace('<h1>','<h2>').replace('</h1>','</h2>').replace('CURRENT CHECKPOINT','PRIVILEGED-STATE CHECKPOINT')
    challenge_rows = []
    for row in ordered:
