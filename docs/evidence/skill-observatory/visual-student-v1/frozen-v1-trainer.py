...
def load_split(root,split,limit):
    ...
    for path in paths:
        with np.load(path) as d:
            length=len(d['action'])
            if 'pickup_end_step' in d:length=min(length,int(d['pickup_end_step']))
...
def main():
    ...
    report={'scope':'Visual pickup action-chunk BC; twofixedRGB cameras+deployable joint/commandfeedback only. Noenv/pose/contact/phase at studentruntime.',
            ...
            'training_rng_seed':2026,'source_sha256':{'policy':sha('src/astrafactory/vision_policy.py'),'trainer':sha(__file__)},
    ...
    for update in range(a.updates):
        ...
        pairs=[(i,int(rng.integers(len(train[i]['action'])))) for i,_ in pairs]
